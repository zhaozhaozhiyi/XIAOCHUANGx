...
| `0.1.7` | Desktop Alpha Skill 编排优化版（候选验证中） | Companion 统一选择、业务 Skill 按需加载、真实 Skill Run Events 与三 Agent 一致性；不包含统一任务和导航改造 |
...
> **小窗 `0.1.7`，Desktop Alpha Skill 编排优化候选版。**
> `0.1.5` 仍是最近正式基线；`0.1.6-rc.3` 是本轮升级/降级验收的最近候选基线。当前工作区已进入 `0.1.7` 候选验证，根 `package.json`、桌面安装包、Web/API/Companion 与必要内部包统一使用 `0.1.7`。该候选必须完成 macOS/Windows 制品、安装态升级降级和回滚验收后才能发布。
