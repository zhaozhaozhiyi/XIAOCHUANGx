...
| P4 | **扩展 Skill 可选** | 按 `surfaceProfile` 选择对应 Catalog Profile；条目以 **Catalog 摘要** 进入 system；全文与 `references/` 由 Agent 经 Kit 或读盘 **按需 Pull**。 |
| P5 | **工具不设门禁** | CLI 自带工具 + 领域服务（数据源等）由 Agent 按任务调用；平台 **不**为每个意图预绑定工具列表。 |
| P6 | **禁止全库灌 prompt** | 任意 Run 不得将 `skills/` 全部 `SKILL.md` 正文塞进 system。 |
| P7 | **可观测、可交付** | `run.started` 暴露 `surfaceProfile`、基座 slug、Catalog Profile、Kit 路径；行为变更优先改 `prompts/`、`skills/`。 |

### 1.3 与非目标

- **非目标：** 每轮 LLM/规则 Router 强制 `augmentSkills[]`（可作为 Desktop Beta **可选提示**，默认关闭）。
...
| Skill Catalog | `catalogProfile` 对应的 Catalog 文件或生成物 | ✅ **仅摘要**（slug + 一行描述） | 可见即可，**不**强制使用 |

**不 Push（默认）：**

- 其他流程 Skill 全文（`skill-ppt-*`、`skill-wr-*` …）
- 未在当前 `catalogProfile` 登记的 Skill
- 「本轮你必须用某某 Skill」的硬编码 Router 结论（除非用户显式指定，Desktop Beta）
...
```text
1. Web / Desktop POST /v1/runs（或 /api/chat → Companion / Runtime）
   binding: { moduleId: "chat", mode: "fast"|"deep" }
   messages, projectId, sessionId, agentId, surfaceProfile

2. normalizeChatMode(mode)  // research → deep

3. resolveChatOrchestration({ surfaceProfile, mode, moduleId: "chat" })
   → baseProcessSkill: skill-qa-fast | skill-qa-deep
   → catalogProfile: desktop | web
   → catalogSlugs: 当前 profile 可见的 slug 列表（静态配置）
   → platformNormSkill: skill-platform-research-norms
   // 无 mandatory augmentSkills

4. stageAgentKitForRun(runId, baseProcessSkill)
   → Desktop: ~/.jlcresearch/agent-kit/runs/<runId>/
   → Web: cloud-runtime 内部 run kit 目录
   // MVP：仅同步基座 references；Web / Desktop 不共享 kit 目录
   // Desktop Beta 可选：catalog/ 下按需预置常用 Skill 副本
...
### 5.1 Skill Catalog Profile（对话可见）

Catalog Profile 是运行面的 Skill 可见面配置。允许共享 `skills/<slug>/` 仓库，但不允许 Web / Desktop 共用同一份可见面配置。

| Profile | 文件（建议） | 使用面 | 说明 |
|---------|--------------|--------|------|
| `desktop` | `skills/catalogs/chat.desktop.json` | `surfaceProfile=desktop` | 可由当前 `skills/chat-catalog.json` 迁移而来 |
| `web` | `skills/catalogs/chat.web.json` | `surfaceProfile=web` | 只登记云端 Runtime 可用的 Skill / 工具 / 模板 |
| `dev-only` | `skills/catalogs/chat.dev.json` | 开发 / 演示 | 可包含临时 gateway 或实验 Skill |

`skills/chat-catalog.json` 若继续存在，只视为历史兼容入口，不允许 Web 在其上直接扩展正式 Skill 可见面。
...
| `requires` | ⬜ | 领域服务或执行面能力；不可用则在对应 profile 中移除或标注 |

**MVP：** Catalog Profile **静态维护**，与 `module-registry` 中 PPT/写作 slug 手工对齐；不做运行时 LLM 路由。Web Sandbox 1.0 上线前，`web` profile 可以先为空或仅登记云端已验收 Skill。

### 5.2 `run.started` 扩展字段（建议）

| 字段 | 类型 | 说明 |
|------|------|------|
| `surfaceProfile` | `"desktop"` \| `"web"` \| `"dev-only"` | 当前运行面 |
| `baseProcessSkill` | string | 基座 slug |
| `platformNormSkill` | string | 横切 slug |
| `catalogProfile` | string | 本轮使用的 Catalog Profile |
...
**不输出：** `mandatoryAugmentSkills`、`routerConfidence`（除非 Desktop Beta 开启可选提示器）。

### 5.3 与 `module-registry` 关系

| 模块 | 绑定方式 |
|------|----------|
| `chat` | `mode` → **仅**基座 `CHAT_MODE_SKILL`；`surfaceProfile` → `catalogProfile` |
| `writing` / `ppt` / … | 仍走各模块 `templateId` / `task`；条目可同时出现在对应 profile 的 **chat Catalog** 供对话 Agent 自选 |

`resolveSkills()` **保持**现有签名；新增 `resolveChatOrchestration()` 专供 `moduleId=chat`，内部调用 `resolveSkills` 取基座，并按 `surfaceProfile` 加载对应 Catalog Profile。
...
| R4 | `run.started` 含 `surfaceProfile`、`baseProcessSkill`、`catalogProfile`、`catalogSlugs`（或 `catalogVersion`）、`agentKitPath` |
| R5 | 修改 `skill-qa-*` / `prompts/platform` 重启 Companion 即可改变对话倾向，无需改 Web 组件 |
| R6 | **禁止**每 Run 自动注入多个 augment 流程 Skill 正文 |
| R7 | Web / Desktop 必须使用不同 Catalog Profile；Web profile 变更不得改变 Desktop 默认可见面 |

### 6.2 应当（S1O / S1P）

| ID | 需求 |
|----|------|
| R8 | `composeSystemPrompt` 与 `userTurn` 分离；CLI 双通道 |
| R9 | Activity 可展示「基座：skill-qa-deep」等轻量标签（非强制列表 augment） |
| R10 | 深度档复杂任务仍可落 `report.md` 等于 `projectId` 根（由基座 Skill 指引，非 Router 强制） |

### 6.3 可选（Desktop Beta+）

| ID | 需求 |
|----|------|
| R11 | `skill_read(slug)` 平台工具：从 Kit `catalog/<slug>` 读 SKILL.md |
| R12 | 用户显式 `@skill` 或 `/slug` 强制加载某 Skill 全文入 Kit |
| R13 | **软提示 Router**：仅当关键词高置信时在 Activity 写「可参考 skill-ppt-deck」，**不**改 system 栈 |
| R14 | Hermes CLI 若自带 `skills_list`/`skill_view`，与平台 Catalog **去重说明**写在 `chat-orchestration.md` |
...
| 可测 | `pnpm skills:verify` 扩展校验各 Catalog Profile 与基座 slug 存在 |
...
| **S1O.2** | `desktop` / `web` Catalog Profile + `composeRunPrompts` 拼 Catalog 段 | system 含当前 profile 摘要列表、无第二 Skill 正文 |
| **S1O.3** | `resolveChatOrchestration` + Companion 接入 | `run.started.orchestrationMode=hybrid-steer` |
| **S1O.4** | UI Activity 可选展示基座 slug | 联调可见 |
| **Desktop Beta** | `skill_read` / `@skill` / 软提示 Router | 独立立项 |
...
| Hermes-agent | 强制扫 Skill 索引；小窗 **不强制**，仅 **建议** 相关时用；正式 Web 中 Hermes 只能作为 Runtime 内部执行器或 `agentId` |
