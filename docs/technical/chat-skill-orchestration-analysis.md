...
4. **Hermes**：量产 Hermes 仅作 CLI 之一；若 CLI 自带 `skill_view`，在 `chat-orchestration.md` 说明与平台 Catalog 共存（Desktop Beta 验收）。
5. **Desktop Beta 可选**：§7 强 Router、`@skill` 用户覆盖、软提示 Router。
