# 桌面壳 Desktop Beta 路线图

| 属性 | 内容 |
|------|------|
| 文档版本 | v0.1 |
| 修订日期 | 2026-06-06 |
| 对应平台版本 | `0.2.0-beta` / Desktop Beta |
| 状态 | 草案（Desktop Beta 主路径；文件名保留历史命名） |
| 上级文档 | [docs/product/PRD-小窗.md](../product/PRD-小窗.md) §1.2、§5.3.7、§10.1、§10.3 |
| 关联 | [desktop-shell.md](../technical/desktop-shell.md)、[folder-import-and-desktop-shell.md](../technical/folder-import-and-desktop-shell.md)、[chat-execution-roadmap.md](./chat-execution-roadmap.md) |

---

## 1. 背景与决策

**2026-06-06 产品决策**：基于"本机单用户 + Companion + agent CLI"产品形态，**Desktop Beta 聚焦桌面端能力**，Web 浏览器版与 Nest 多用户后台整体推到 **Web Sandbox 下一大版本**。

Desktop Beta 桌面壳能力是 `0.2.0-beta` 主路径的支柱，包含 **6 项**增量（按优先级排序）：
...
> 完整设计：[folder-import-and-desktop-shell.md §4.3](../technical/folder-import-and-desktop-shell.md#43-桌面-hmacv11)
...
| Web 浏览器路径 | 不带 token；`fromTrustedPicker: false`；保留为当前工程降级路径（Desktop Beta 不作为正式 Web 产品路径） | `companion/src/projects/store.ts` |
...
- [x] 网络错误时静默失败，不阻塞主功能（`auto-updater.ts` 走 'error' 状态透出 hint 不抛；状态见本节与 §10 D1.5）
...
## 11. 不在 Desktop Beta 桌面壳范围

| 项 | 推到 |
|----|------|
| Linux 安装包 | Desktop Beta+（Win/mac 优先） |
| 多窗口 / 多 instance | Web Sandbox 或后续 Desktop 增强 |
| 模式 A 云端工作区切换 | Web Sandbox |
| 设置面板独立窗口 | 后续 Desktop 增强 |
| 浏览器扩展 / Web Push | Web Sandbox |
| 跨设备会话同步 | Web Sandbox（依赖 Nest） |
| 离线模式 | Desktop Beta+ |
...
| 版本 | 日期 | 说明 |
|------|------|------|
| v0.1 | 2026-06-06 | 初稿；基于 Desktop Beta 范围调整决策（Web/Nest 推 Web Sandbox 下一大版本 + 桌面优先） |
