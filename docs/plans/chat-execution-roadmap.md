...
| 属性 | 内容 |
|------|------|
| 文档版本 | v1.6（对齐 PRD **v4.1**） |
| 修订日期 | 2026-06-26 |
| 目标 | **`0.1.0-alpha` / Desktop Alpha**：对话内容（`parts[]`、自动问答策略、侧栏状态、Turn 吸顶）+ **Electron 桌面壳** + Companion 真 CLI；六模块导航可见（对话 / 写作 / PPT / 3D绘图 / 视频 / 推演），但 Alpha 主验收只覆盖对话与桌面壳；翻译、会议纪要、知识库不进入当前导航 |
| 关联 | PRD §5.3.2.2、§5.3.7、§6.10 **F-RT-003/008/007c**；[chat-core-architecture.md](../technical/chat-core-architecture.md)；[folder-import-and-desktop-shell.md](../technical/folder-import-and-desktop-shell.md)；[chat-message-parts.md](../technical/chat-message-parts.md)；[companion-api.md](../technical/companion-api.md) |
| 原则 | **先执行闭环（S1）→ Prompt 外挂与 Agent Kit（S1P）→ 工作区真树（S2）→ P1 体验（S3）** |

---

## 总览（`0.1.0-alpha` 节奏，建议 8～10 周）

| 阶段 | 周期 | 交付物 | `0.1.0-alpha` |
|------|------|--------|-----|
...
**与 `0.1.0-alpha` / Desktop Alpha 的对应：**
...
| S4 | §5.3.7 Electron 桌面壳（**Desktop Alpha P0**） |
| S3 | F-QA-009 P1、F-QA-010 |

---

## 进度快照（2026-06-26 · Desktop Alpha 收口）

> 与 PRD [§1.2 当前实现进展](../product/PRD-小窗.md#12-当前范围) 及 [mvp-closure-checklist](./mvp-closure-checklist.md) 一致。
...
| S1.2 | ✅ | 自动化 SSE 与 Web 回归已验；`stable long-session` / `persists committed outline` / `opens workspace file links` PASS（2026-06-26） |
| S1.3 | ✅ | Turn 吸顶已修复并接入 `ChatTurnList`；`useActiveTurn` 只将覆盖 sticky 锚点且用户问仍有空间吸顶的 Turn 标为 active，避免上一轮滚出父容器后仍吸顶；`stable long-session` PASS（2026-06-26） |
| S1.4 | ✅ | `runtimeStatusTitle` →「Companion 已连接 · {agentId} CLI 可用」；GUI DOM 复检为「Companion 已连接 · codex CLI 可用 · cli」 |
| S1.5 | ✅ | cancel Run 已接；2026-06-26 修复快速取消竞态，复测 `/cancel` → `run.cancelled` 且无残留 CLI 进程 |
...
| **`0.1.0-alpha`** | ✅ | 核心链路收口：`pnpm mvp:verify` PASS、关键 Web 回归 PASS、Desktop pack:dir + 冷启动 PASS；剩余为签名/公证与 Desktop Beta 体验项 |
...
| **六模块导航** | ✅ | 产品文档与代码现已收口到对话 / 写作 / PPT / 3D绘图 / 视频 / 推演；其中对话 / 写作 / PPT 为 Desktop Alpha / Beta 主验收线，3D / 视频 / 推演按子线验收 |
...
| S1.1d | 超时与取消 | `manager.ts`, cancel API, `run-agent.ts` | ✅ 2026-06-26 复测并修复快速取消竞态 |
...
- [x] 停 Companion → 明确 `agent_unavailable`，非模拟回复（checklist D7，2026-06-26 复测：`/api/chat` HTTP 422）
...
**验收：** 见 [chat-message-parts.md §11](../technical/chat-message-parts.md) P0 四条。

---

### S1.3 Turn 吸顶联调（F-QA-009）

代码部分已有（`useActiveTurn`），但当前 `ChatTurnList` 未消费 active turn 结果；本项仍需修复 + 验收。
...
| 文件 | 状态 |
|------|------|
| `HermesStatusBadge.tsx` | ✅ 已接 Companion/Agent 状态文案；需 UI 文案最终走查 |
...
| ID | 任务 | 文件 |
|----|------|------|
| S1.5a | Web 停止 | `ChatComposer.tsx`, `useChatRunController.ts` ✅ |
| S1.5b | Companion cancel / interrupt | `web/src/lib/companion/cancel-run.ts`、`companion/src/routes/runs.ts` ✅ |
...
| `prompts/platform/mode-hints.md` | 自动问答策略说明 | ✅ |
| `prompts/platform/workflow.md` | 通用工作流 | ✅ |
| `skills/skill-platform-research-norms/` | 横切规范 | ✅ |
| `skills/skill-qa/`、`skill-qa-fast/`、`skill-qa-deep/` | 对话流程 Skill（默认自动，fast/deep 兼容旧会话） | ✅ |
...
- [x] `pnpm skills:verify` 仍通过；且 `prompts` 被加载（代码路径已接，需按当前环境重跑）
- [x] 日志 / `run.started` 可见 `injectedSkills` + platform 文件列表（代码已完成）
...
- [x] 用户工作区内**无** `.jlc-skills` / `agent-kit` 文件夹（代码使用 `~/.jlcresearch/agent-kit/runs/<runId>/`）
- [x] 并发两 Run 时 `runs/<runId>` 隔离（代码按 `runId` 分目录）
...
**验收：** 对话默认由 `mode=auto` → `skill-qa` → `skills/` + `prompts/platform` 驱动；`fast/deep` 仅为旧会话/API 兼容。
...
**规格：** [chat-core-architecture.md](../technical/chat-core-architecture.md)
...
| S1O.1b | 基座 Skill 补充 Catalog 说明 | `skills/skill-qa/SKILL.md`（兼容 `skill-qa-fast/deep`） | 🔶 待接入 compose 后验收 |
...
**非范围：** §7 mandatory Router（见 [chat-skill-orchestration-analysis.md](../technical/chat-skill-orchestration-analysis.md)）→ Desktop Beta。

---

### S1P DoD（Prompt 交付闭环）

- [x] 客户仅替换 `JLC_SKILLS_DIR` / `JLC_PROMPTS_DIR` 指向定制包即可切换行为（代码支持）
- [x] PRD F-RT-003 验收条目中与 Prompt 相关的项可勾选（代码支持，仍建议重跑 smoke）
...
### S2.3 文件夹导入（F-RT-007c，PRD v4.1 / Desktop Alpha）

> 设计全文：[folder-import-and-desktop-shell.md](../technical/folder-import-and-desktop-shell.md)
...
- [x] 手填 `~/Projects/foo` 导入后真树与磁盘一致（2026-06-06 已验）
- [x] Agent 写入文件出现在 `foo` 目录（2026-06-06 已验）
- [x] 无 `showDirectoryPicker` 误导流程（代码主路径为手填 / Desktop IPC）
- [x] 错误路径提示清晰（不存在 / 不在主目录下）
...
> 详细需求：[chat-agent-output-ux.md](../design/chat-agent-output-ux.md)
...
- [x] 步间间隔 ≥3s 显示「思考 Ns」；快速模式可隐藏或提高阈值。
- [x] 研究 mock 在正文后出现成品卡，主交付 + ≥2 附件，点击 `openFileAt` 打开。

**建议顺序：** S3.5b → S3.5a → S3.5c。

---

## S4 — 桌面壳 Electron（Desktop Alpha P0，优先于纯 Web 手填路径）

> 摘要：[desktop-shell.md](../technical/desktop-shell.md) · PRD v4.1 §5.3.7
...
| S4.4 | HMAC + `/v1/desktop/register` | 防伪造 `baseDir` | ✅ 代码已完成；需打包态复测 |
| S4.5 | 托盘 / Companion 安装检测 | 首次启动引导 | ✅ 代码已完成；需 Desktop Beta 体验验收 |

**Desktop Alpha 验收（2026-06-06 收口）：**

- [x] 桌面应用启动进入对话；与浏览器同一 Web 产物
- [x] 选目录后 Web 仅见 `projectId` + `pathSummary`
- [x] 选目录 → 新建会话 → Companion 真树 + Agent 写入课题目录

> **Desktop Beta 桌面壳完整路线图：[desktop-v1.1-roadmap.md](./desktop-v1.1-roadmap.md)（历史文件名保留，2026-06-06 立）**  
...
| ⬜ | S4.2～S4.4 | HMAC / 托盘（Desktop Beta） | 2d |

**推荐实施顺序（`0.1.0-alpha`）：** **S4.0～S4.2**（桌面壳联调）∥ **S1.0～S1.2**（真流）→ S2.0/S2.3 → S1P（可选）→ S3 P1。

**`0.1.0-alpha` 完成定义：** 桌面壳选目录 → 新建对话 → Codex 真流 → `parts[]` 可见；侧栏状态 + Turn 吸顶可演示；六模块导航入口可见但无写作 / PPT / 3D / 视频 / 推演正式闭环验收项。
...
| 领域 | 路径 |
|------|------|
| `0.1.0-alpha` / Desktop Alpha | `docs/product/PRD-小窗.md` §2.4、§10.2、§5.3.7 |
| 文件夹导入 / 桌面壳 | `docs/technical/folder-import-and-desktop-shell.md`、`desktop-shell.md` |
...
| Agent 输出 UX（思考/探索/成品） | `docs/design/chat-agent-output-ux.md` |
