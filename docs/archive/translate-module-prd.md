...
| 日期 | 2026-06-06 |
| 状态 | 历史草案（当前不进入现行模块范围） |
| 关联 | PRD-小窗 §6.3；[writing-module-prd.v2.md](./writing-module-prd.v2.md)；[ppt-module-prd.v2.md](./ppt-module-prd.v2.md)；[chat-core-architecture.md](../../technical/chat-core-architecture.md) |

> 本文档保留为历史设计草案，用于记录 2026-06 时点的翻译模块方案。**当前产品现行范围仅保留对话 / 写作 / PPT 三模块**，翻译不进入当前 Desktop Alpha / Beta 主线。
...
| BYOK 翻译 API（如 DeepL/Google）调度面板 | Desktop Beta+；`0.1.0-alpha` 由 Agent + Skill 走通用模型路径 |
| 快速/深度模式切换 | 翻译按 Skill 自带的"快报/详细"分支即可，UI 不再区分 |
| 多人协作 / 翻译记忆库（TM）/ 术语库管理 | Web Sandbox+ |
...
> 代码状态更新（2026-06-22）：以下状态基于当前仓库代码静态核对；本轮未执行真实 Companion/Agent 翻译链路、`pnpm skills:verify` 或 UI 手测。涉及真实文档翻译落盘、SSE 真流、历史侧栏展示的项目，仍需按真实链路验收。

### P0（Desktop Beta 启动）

| # | 项 | 验证 | 状态 |
|---|-----|------|------|
| T1 | 点击「翻译」进入对话界面，**非表单页** | 目测 | 已完成：`/translate/new` 渲染 `ChatHome surfaceModuleId="translate"` |
| T2 | 新建会话默认加载 `skill-tr-text` | `run.started` 元数据 `processSkill` | 代码已完成，待验收：`MODULE_CHAT_SURFACES.translate.defaultProcessSkill = skill-tr-text` |
| T3 | Composer 下拉可切换 text / doc / polish 三个 Skill，切换后再发会换 `processSkill` | 看下次 `run.started` | 代码已完成，待验收：`TRANSLATE_SKILL_OPTIONS` 与 `translateTemplateId` 透传链路存在 |
| T4 | 翻译对话过程与聊天一致（流式 / parts / 工具） | 同聊天 D1-D4 | 代码已完成，待验收：共享 `ChatHome/ChatThread/useChatSend` 链路；真实 SSE 未在本轮验证 |
| T5 | 文档翻译 Agent 产出 `.md` 文件到工作区翻译子目录 | 工作区可见 `XIAOCHUANG/翻译/…/*.md` | 代码已完成，待验收：翻译模块 `workspaceSegment="翻译"`；真实文档翻译落盘未在本轮验证 |
| T6 | 翻译会话出现在历史侧栏中 | 侧栏可见 surfaceModuleId="translate" 的会话 | 代码已完成，待验收：历史按 `surfaceModuleId` 分组；UI 手测未在本轮执行 |
| T7 | `pnpm skills:verify` 通过；`loadSkill` 3 个 `skill-tr-*` 均能加载 | CI / 本地脚本 | 未完成/待验收：3 个 `SKILL.md` 已存在，但本轮未跑验证脚本 |

### P1（体验优化）

| # | 项 | 说明 | 状态 |
|---|-----|------|------|
| T8 | 文本翻译至少给 2 个备选译文 + 说明 | Skill 强制；可在样本对话中目检 | 代码已完成，待验收：`skill-tr-text` 已写入约束，需样本对话目检 |
| T9 | 文档翻译保留 H1/H2/H3 层级与表格 | 抽样文档对照 | 代码已完成，待验收：`skill-tr-doc` 已写入约束，需真实文档抽样 |
| T10 | 译文润色给出 ≥ 3 条具体修改点 + 修改对照 | 抽样润色任务目检 | 代码已完成，待验收：`skill-tr-polish` 已写入约束，需真实样本 |
| T11 | 翻译任务目录命名符合 `XIAOCHUANG/翻译/{YYYY-MM-DD}/{标题简写}/` | 工作区检查 | 代码已完成，待验收：Companion 默认任务目录支持模块 segment；翻译真实落盘未在本轮验证 |
...
| Composer 下拉 Skill 数 | 3（通用/公文/会议纪要，当前仍为写作 Skill 选项，不等同会议模块） | 17+（与 Open Design 同步） | 3（文本/文档/润色） |
...
## 9. 实施任务（Desktop Beta 启动批次）

| # | 任务 | 状态 |
|---|------|------|
| 1 | 3 个 SKILL.md（`skill-tr-text/doc/polish`）| 已完成：对应 `SKILL.md` 已存在 |
| 2 | 路由 `/translate/{,new,[id]}/page.tsx` | 已完成：当前 3 个页面文件已存在 |
| 3 | `navigation.ts` 加 `translate` NAV_MODULES 项 | 已完成：侧栏已有一级「翻译」入口 |
| 4 | `module-registry.ts`：`ModuleId` / `DomainService` / `MODULE_REGISTRY` / `TRANSLATE_TEMPLATE_SKILL` / `resolveSkills` 分支 | 已完成：翻译模块、目录 segment、template skill 映射已存在 |
| 5 | `module-chat-config.ts`：`MODULE_CHAT_SURFACES.translate` / `TRANSLATE_SKILL_OPTIONS` / `skillPicker` | 已完成：翻译 surface 与 Skill 下拉配置已存在 |
| 6 | `companion/run.ts`：`surfaceModuleId="translate"` 分支、`processSkill`、`binding`、`timeoutProfile` | 代码已完成，待验收：Web companion run adapter 已有 translate 分支；真实 Companion run 未在本轮验证 |
| 7 | `chat-catalog.json` / `skills/README.md` 更新 | 已完成：3 个 `skill-tr-*` 已注册并在 README 中列出 |
| 8 | 全链路 `translateTemplateId` 透传（Composer→pending→sendMessage→hermes→companion）| 代码已完成，待验收：前端、Hermes client、Companion run adapter 均有字段；真实 run 元数据未在本轮验证 |
| 9 | PRD-小窗 §6.3 / §4.2 加 v2 替代头；功能清单同步 | 代码外文档项，待复核：本轮未重新逐段核对 PRD 对应章节 |
| 10 | `pnpm skills:verify` PASS + `loadSkill` 3 个新 skill 验证 | 未完成/待验收：本轮未跑脚本 |
| 11 | **端到端 SSE 真流验证** | 未完成：留 P0 验收用 |
| 12 | 文档翻译 BYOK 接口（DeepL / Google API） | 未完成：Desktop Beta+ |
| 13 | 翻译记忆库 / 术语库管理 | 未完成：Web Sandbox+ |
...
| `agentPrimaryPath: true` | 是 | 与 PRD-小窗 §6.3.4 D-29 兼容；BYOK API 是 Desktop Beta+ 增强而非替代 |
| `producesWorkspaceArtifacts: true` + `workspaceSegment="翻译"` | 是 | 与 §5.3.2.1a 注册表保持一致 |
| 不再为「文档翻译 / 文本翻译」分别建路由 | 是 | 由 Skill 切换承载；UI 一致性 |
| 端到端 SSE 真流验证留到 Desktop Beta 验收 | 是 | 本批次为"启动验证"，先把绑定 / 注册 / 路由 / Skill 上齐 |
