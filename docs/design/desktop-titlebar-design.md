...
| 关联 | [桌面壳技术摘要](../technical/desktop-shell.md)、[UI 设计规范-Claude 风格](./UI设计规范-Claude风格.md)、[文件夹导入](../technical/folder-import-and-desktop-shell.md) |
...
#### 改动 3 — 跟随系统主题切换 overlay 配色（Desktop Beta，可选）
...
| 打开项目 | `Ctrl+O` | 自定义 accelerator → `pickAndImportFolder` IPC（Desktop Beta） |
| 新对话 | `Ctrl+N` | 自定义 accelerator → 渲染层事件（Desktop Beta） |
...
| # | 风险 | 应对 |
|---|------|------|
| R1 | `titleBarOverlay.color` 不支持 CSS 变量，深色模式切换需要主进程介入 | `0.1.0-alpha` 固定浅色；深色模式排到 Desktop Beta 时实现 §6.1 改动 3 |
...
| 12 | 截图入 PRD §12.5.X 实现快照 | `docs/product/PRD-小窗.md` | 0.3h | — |
