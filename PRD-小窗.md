...
| 一级模块 | 产品线 | MVP（v3.0） | V1.1+ |
|----------|--------|-------------|-------|
| **对话** | works | **全文验收**（§6.1、F-QA-001/007/008/009；F-QA-010 P1；F-QA-002～005 延后） | 数据源、图表、多信源、可溯源 |
| **桌面壳** | 交付形态 | **Electron 最小集**（§5.3.7）：加载同一 `web/` UI + 系统选目录 `pickAndImportFolder` | HMAC、托盘、自动更新、Companion 捆绑安装 |
| 会议纪要 | works | 占位页，不验收 | 全文 §6.4 |
| 知识库 | works | 占位页，不验收 | 全文 §6.5 |
| 写作 | works | 占位/骨架，不验收 | 全文 §6.6 |
| PPT | works | 占位/骨架，不验收 | 全文 §6.9 |
| 翻译 | works | 占位页，不验收 | 全文 §6.3 |
| 账号与登录 | 横切 | §6.12 MVP | 微信登录等 |
| 全局设置 | 横切 | §6.11 MVP 子集 | §6.11.2～6.11.5 |
| 智能体运行时 | 横切 | §6.10（对话所需：F-RT-001～005、007/007b/007c、008；**009 基线**；**多 CLI / 多模型适配为 MVP 核心验收项**） | F-RT-006 完整、模式 A 完整、**F-RT-002 + F-RT-009 BYOK 增强** |
...
| 沙箱项目 | `workspaceKind = sandbox`：用户未选择本地文件夹时，Companion 在托管目录 `<dataDir>/projects/<projectId>/` 自动建项；仍有完整 `projectId` 与 Agent `cwd` |
| 本地绑定项目 | `workspaceKind = local_bound`：经**文件夹导入**登记 `baseDir`（桌面系统选目录或 Web 手填路径 + Companion 校验），Agent **直接在原目录**读写，不复制到沙箱 |
...
| UI「不使用项目」 | 对话输入区「进入项目工作」中的选项；**执行层仍绑定 `sandbox` `projectId`**（见 §12.5.3） |
...
- [ ] 始终有 `projectId`：未选文件夹 → `sandbox`；桌面选目录 → `local_bound` + **新建会话**（§5.3.2.1、F-RT-007）
...
|-----------------|----------|-------------------|-----------|
| `sandbox` | 新建对话且未「进入项目工作」选文件夹 | Companion 托管：`<dataDir>/projects/<projectId>/`（`dataDir` 实现阶段定义，可配置） | 无（由 Companion 解析） |
| `local_bound` | 「添加新项目 / 打开文件夹」→ **文件夹导入** | 用户授权的绝对路径 `baseDir`（`realpath` 后落库） | 必填；**不复制**目录内容 |
| `cloud` | 模式 A 或无 Companion 降级 | OSS/云端路径，与 `projectId` 映射 | 平台内部，不对用户暴露 |
...
| 新建对话默认 | **模式 B**（Companion 可用）：默认创建 **`sandbox`** 的 `projectId`；**模式 A**（无 Companion 或企业 cloud-only）：默认创建 **`cloud`** 的 `projectId`（平台/OSS，不经 Companion） |
| 复用项目 | 用户可为**新对话会话**选择已有 `local_bound` 或 `sandbox` 的 `projectId`（共享目录）；**不等于**改绑当前会话 |
| 分支会话 | 从某条消息「分支新会话」→ 新 `sessionId`，**继承**原会话的 `projectId`（固定绑定规则不变） |
| Agent `cwd` | 单次 Run/任务在**整个 Project 根**下执行；同 `projectId` 下多对话会话**共享**该根目录（与 Open Design 一致） |
| 并发（待实现定义） | 同 `projectId` 多会话并行 Agent 时，实现层须串行化写盘或文件锁，避免产出互相覆盖（见 OQ-20） |
| 沙箱生命周期 | 沙箱 `projectId` 可与历史对话会话关联保留；企业可配置闲置清理策略（实现阶段） |
| V1.1 可选 | 企业配置 `defaultWorkspaceRoot` 时，新建**本地绑定**项目可在默认根下自动 `mkdir` 子目录（仍记 `local_bound`） |

```mermaid
flowchart LR
  NewChat[新建对话]
  Sandbox[创建 sandbox projectId]
  Session[创建 session 绑定 projectId]
  Optional[用户: 进入项目工作]
  Local[创建或选择 local_bound projectId]
  NewSession[新建 session 绑定新 projectId]

  NewChat --> Sandbox --> Session
  Session --> Optional
  Optional --> Local --> NewSession
```
...
| 仅浏览器（模式 A） | 无 Companion 时云端降级 | **V1.1**（非 MVP 必验） |
...
| 安全边界 | `baseDir` 须在用户主目录下（如 `~/` 展开）；拒绝 `dataDir` 内路径、系统敏感路径；每次写盘仍走 `resolveSafe` / 越权拒绝 |
| 与沙箱区分 | UI「不使用项目」→ 执行层仍为 `sandbox`；选本地项目 → `local_bound` + 真实目录树（§12.5.3） |
...
| 方法 | 路径 | 说明 |
|------|------|------|
| `POST` | `/v1/projects/import-folder` | Body：`{ name?, baseDir, skillId? }` → 创建/登记 `local_bound` `projectId`，`realpath(baseDir)`；返回 `{ projectId, name, workspaceKind, pathSummary }` |
| `POST` | `/v1/projects/ensure` | Body：`{ projectId, workspaceKind, name, baseDir }` → 固定 ID 登记（用于演示项目列表与 MOCK 对齐）；**不能**替代受信选目录的安全语义 |
| `GET` | `/v1/projects/:projectId/tree` | 列 `baseDir` 下真树（已有） |
...
| 创建 | 用户进入上述模块并发起生成任务时，创建或关联 **对话会话** + **`projectId`**（规则见 §5.3.2.1） |
| 固定绑定 | 已创建对话会话的 `projectId` **不可修改** |
| 切换文件夹 | 「进入项目工作」选本地绑定 → **新建对话会话** + 新建或复用 `local_bound` 的 `projectId` |
| 分支 | 分支新对话会话 **继承** 原 `projectId` |
| 跨模块跳转 | 「发送到对话」等：默认 **新建对话会话** + 新建 **`sandbox`** `projectId`（可二期支持用户选择已有 `local_bound` 项目） |
| 模块内历史 | 「我的文稿」「纪要历史」等为**业务记录**（DB/索引）；与工作区文件**并存**：验收要求 CLI 产出至少 1 个文件于 `projectId` 根（F-RT-004） |
...
- 输入区**下方**提供 **「进入项目工作」**（`ProjectWorkPicker`，§6.10 F-RT-007）：切换到**本地绑定**项目须**新建会话**；选「不使用项目」时 UI 为无项目组、执行层仍为沙箱 `projectId`（§12.5.3）；支持 `@` 提及当前项目根内文件
...
- UI 选「不使用项目」→ 侧栏历史归入「无项目」组；`resolveWorkspaceProjectId` 仍映射为 **`sandbox`**（§12.5.3）。
...
|------|------|
| **入口** | 「新建纪要」 |
| **项目与会话** | 发起纪要生成任务时创建**对话会话** + `projectId`（模式 B 默认 `sandbox`，模式 A 默认 `cloud`）；转写稿、结构化纪要等 CLI 产出写入该 `projectId` 根；业务「纪要历史」索引与之关联 |
| **主路径** | 上传音视频 → 校验格式与时长 → 异步转写与理解 → 结果页分 Tab 展示概要 / 大纲 / QA / 全文转写 |
| **产出物** | 四类内容均可复制；支持合并导出（PDF/Word/Markdown 至少一种）；转写全文可搜索 |
| **历史** | 「纪要历史」列表含状态（处理中/成功/失败）、创建时间、标题（用户可编辑）；失败任务可重试或重新上传 |
...
| **沉淀与延伸（可选）** | 从知识库问答「发送到对话」：新建**对话会话** + 默认新建 **`sandbox`** `projectId`，首条消息携带库内引用上下文（§6.0.3） |
...
| **项目与会话** | 进入撰写流程时创建**对话会话** + `projectId`；大纲 MD、章节文稿等 CLI 产出写入 `projectId` 根；「我的文稿」为业务记录 + 工作区文件并存（§6.0.3） |
...
| `moduleId` | 一级菜单 | 领域服务（L2，主路径） | 流程 Skill 绑定键 | 模板资产包 | Agent 主路径 |
|------------|----------|------------------------|-------------------|------------|--------------|
| `chat` | 对话 | 数据源连接器、图表引擎；可选 RAG | `mode`: `fast` → `skill-qa-fast`；`deep` → `skill-qa-deep`（`research` 仅作 API 别名→`deep`） | — | ✅ |
| `meeting` | 会议纪要 | ASR/转写 | `task`: `summary` → `skill-mm-summary` | 可选纪要版式 `tpl-mm-*` | ✅（结构化阶段） |
| `knowledge` | 知识库 | 文档存储、索引、检索、RAG | `task`: `kb-qa` → `skill-kb-qa`（V1.1，库内问答） | — | 子场景 ✅ |
| `writing` | 写作 | 数据源连接器、图表引擎 | `templateId` → `skill-wr-{templateId}` | `assets/template.*`  per 模板 | ✅ |
| `ppt` | PPT | 幻灯片引擎 | `task`: `deck` → `skill-ppt-deck` | `tpl-ppt-*` 母版/路演版式 | ✅ |
| `translate` | 翻译 | 翻译引擎、文档解析 | MVP **无**流程 Skill；V1.1 可选 `skill-tr-polish`（润色/术语） | — | ❌（API 主路径） |
| — | 全局设置 / 登录 | 鉴权、配置存储 | — | — | ❌ |
...
`bindingKey` 示例：对话 `{ mode: "fast" }` 或 `{ mode: "deep" }`；写作 `{ templateId: "policy" }`；纪要 `{ task: "summary" }`。
...
| 项 | 要求 |
|----|------|
| 新建对话 | 创建 `session` 时同时创建或关联 `projectId`；**模式 B**：默认 `workspaceKind = sandbox`；**模式 A**：默认 `workspaceKind = cloud` |
| 打开/复用项目 | 可为新对话会话选择已有 `sandbox` / `local_bound` / `cloud` 的 `projectId`（共享项目根目录） |
| 「进入项目工作」 | 展示**当前对话会话**已绑定的项目摘要；选择另一**本地绑定**文件夹 → **新建对话会话** + 新建或复用 `local_bound` 的 `projectId` |
| 切换项目 | **禁止** PATCH 已创建会话的 `projectId`；换文件夹 = 新 `sessionId` |
| 分支会话 | 新 `sessionId`，**继承**原 `projectId` |
| `@` 提及 | 当前 `projectId` 根目录内文件可检索并附加（`sandbox` / `local_bound` / `cloud` 均适用，依模式解析树） |
| 工作区 UI | 文件树 = 当前对话会话 `projectId` 根；沙箱显示「临时工作区」等文案 |

**验收标准：**

- [ ] 模式 B：新建对话无选文件夹亦可产生 `sandbox` `projectId` 且可写托管目录
...
| 本地绑定 | **文件夹导入**（§5.3.2.2）：Web 手填 `baseDir` → `POST /v1/projects/import-folder`；桌面壳主进程选目录 + HMAC（§5.3.7）；`POST /v1/projects/ensure` 仅用于演示/MOCK 对齐 |
| 安全 | `realpath` + 主目录边界；`local_bound` 禁止指向 `dataDir` 内路径；渲染进程不持有受信路径明文 |
...
| 本地路径 | `local_bound` 须用户显式授权 `baseDir`；`sandbox` 仅限 Companion 托管目录；拒绝系统目录等（规则实现阶段定义） |
...
| 沙箱项目 | UI 标注「临时工作区」或等价文案，避免用户误以为文件在其课题盘根目录 |
...
| 版本 | 范围摘要 | 目标时间（待排期） |
|------|----------|-------------------|
| **MVP（v3.0）** | **对话内容** + **Electron 桌面壳** + Companion/CLI + 登录 + 全局设置子集；其它模块占位 | T0 + 8～10 周（建议） |
| **V1.1** | 会议纪要、知识库、写作、PPT、翻译；数据源/图表/多信源；模式 A；桌面壳 HMAC/托盘/更新 | T0 + 16 周 |
| **V2.0** | 知识库企业共享 + API 开放 | 待规划 |
...
| **V1.1（Phase 2）** | 006 终端；HMAC/托盘；纪要/知识库/写作/PPT/翻译；模式 A 子集；F-SET 扩展 | 企业扩展 CLI；内网拓扑 A1 |
| **V2.0（Phase 3）** | 云端工作区完整；知识库企业共享 | 模式 A 完整；运维批量升级 CLI |
...
| OQ-17 | 企业 `defaultWorkspaceRoot` 下自动建子目录是否纳入 MVP（V1.1 候选） | 产品 + IT |
| OQ-18 | 模式 A 是否在 V1.1 才强制支持写作/PPT | 产品 |
| OQ-19 | 纪要/文稿「业务历史」与工作区文件的权威源：以 DB 为准还是以 `projectId` 目录为准 | 产品 + 研发 |
...
| 编号 | 结论 |
|------|------|
| — | **始终有 `projectId`**：未选文件夹 = `workspaceKind = sandbox`（Companion 托管目录）；选文件夹 = `local_bound`（`baseDir`） |
| — | **会话固定绑定**：创建会话时写入 `projectId` 且不可修改；**切换项目 = 新建会话** |
| — | Agent `cwd` = 整个 Project 根；同 `projectId` 下多会话共享该目录 |
| OQ-15 | **已决**：切换研究项目须**新建对话会话**；**禁止**改绑当前会话 `projectId`；分支会话**继承**原 `projectId` |
...
| 知识库 → 发送到对话 | 新建对话会话 + 默认 `createProject: { workspaceKind: sandbox }` |
...
| 历史分组 | `useChatHistory` 按 `projectId`；`NO_PROJECT_ID` →「无项目」 | §4.3 |
| 会话状态点 | `runStatus` + `lastReadAt`；`getSessionIndicator`；发送/打开会话时更新 | F-QA-008 |
| 模块注册表 | `lib/module-registry.ts` 与 §6.10.1a 一致 | F-RT-003 |

#### 12.5.3 「不使用项目」与沙箱映射（已决）

| 层 | 值 | 说明 |
|----|-----|------|
| UI / 侧栏分组 | `projectId = none`（`NO_PROJECT_ID`） | 展示「不使用项目」「无项目」 |
| 发送 payload | 可为 `none` |  composer 传出 |
| 工作区 / @ / 执行 | `resolveWorkspaceProjectId` → `sandbox-default` | **始终有 sandbox `projectId`**，满足 §5.3.2.1 |
| 持久化（项目绑定） | `localStorage` `jlc-chat-project-{sessionId}` | 消息正文见 Companion `sessions/*.json`（§8.6） |

**产品文案：** 对用户说明「未绑定本地文件夹时，文件落在**临时工作区**（Companion 托管）」，避免误解为「无工作区」。
...
| D-04 | 未选文件夹 = 沙箱 projectId | UI 显示「不使用项目」但执行映射 sandbox | **已决** §12.5.3 |
