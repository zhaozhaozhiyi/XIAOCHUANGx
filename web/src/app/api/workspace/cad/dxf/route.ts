...
export async function GET(request: Request) {
  ...
  try {
    ...
    tempDir = await mkdtemp(join(tmpdir(), "jlc-scad-dxf-"));
    const inputPath = join(tempDir, "projection.scad");
    const outputPath = join(tempDir, "drawing.dxf");
    await writeFile(inputPath, createDxfProjectionSource(source), "utf8");
    ...
    const rawDxf = await readFile(outputPath, "utf8");
    return Response.json({
      mime: "application/dxf",
      encoding: "utf8",
      content: normalizeOpenScadDxf(rawDxf),
      compiler: "openscad",
      engine: "openscad-cli",
    ...
    return Response.json(
      {
        error: e.killed ? "openscad_timeout" : "openscad_dxf_failed",
