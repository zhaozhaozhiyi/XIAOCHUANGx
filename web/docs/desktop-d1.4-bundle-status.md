# D1.4 Companion 捆绑 · 实施状态（2026-06-08）
...
| 关联代码 | `companion/package.json`、`scripts/prepare-companion-bundle.mjs`、`apps/desktop/electron-builder.yml`、`apps/desktop/src/main/companion-supervisor.ts`、`apps/desktop/build/entitlements.mac.plist` |
...
唯一被牺牲的是"Companion 单独成可执行文件给非桌面用户用"。但 V1.1 形态明确是"本机单用户 + 桌面壳 + Companion"（见 [memory: V1.1 桌面单机](file:///C:/Users/74179/.claude/projects/D--XIOACHUANGPRO/memory/v1.1-scope-desktop-only.md)），不存在"裸 Companion 客户"，这个牺牲不亏。
...
| supervisor 打包态走 bundle 路径 | ✅ | 代码读检查通过；运行验证依赖手测 `pack:dir` |
| `pnpm --filter @jlc/desktop pack:dir` → 装后即可用 | ⏳ 待手测 | 需 Electron prebuild 拉取 ~200MB |
| 卸载清理 + 用户数据保留 | ⏳ 待手测 | 同上 |
