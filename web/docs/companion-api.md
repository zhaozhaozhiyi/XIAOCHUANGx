...
> **对齐：** PRD **v3.0 MVP** §8.5、§5.3.2.2；[folder-import-and-desktop-shell.md](./folder-import-and-desktop-shell.md)；TypeScript 类型见 `web/src/lib/companion/types.ts`。
...
列出本机可见项目（`sandbox` + 用户已 `local_bound`）。

### `POST /v1/projects`

**仅创建沙箱**（MVP 推荐本端点只接受 `sandbox`）：
...
**幂等（实现二选一，须在 release notes 写明）：** 同 `realpath(baseDir)` 返回已有 `projectId`，或每次新建 `projectId`。
