...
> PRD：**v3.0 MVP** §5.3.7（桌面壳为推荐交付形态，非 V1.1 可选项）；技术方案：[技术方案.md](../../技术方案.md) §3.4。
...
| 不推荐 | 原因 |
|--------|------|
| Tauri（MVP 主路径） | 无 OD 参考，Rust 主进程重写成本高 |
...
| `getCompanionHealth` | 渲染 → 主 | 代理 `GET /v1/health`（✅ MVP） |
| `openPathInFinder` | 渲染 → 主 | 仅 `fromTrustedPicker` 项目（V1.1） |
...
- **Companion**：MVP 仍独立安装/启动；V1.1 捆绑安装器或首次启动检测。
