...
test.describe("video module P0", () => {
  ...
  test("starts a web video session and shows P0 deliverables", async ({
    ...
    await page.getByRole("button", { name: "客户高层", exact: true }).click();
    await page.getByPlaceholder("例如：60s、90s、3 分钟以内").fill("60s");
    await page.getByRole("button", { name: "16:9 横屏", exact: true }).click();
    await page
    ...
    await expect(
      page.getByRole("button", { name: "在文件夹中显示 presentation/ 网页视频项目" }),
    ).toBeVisible();
